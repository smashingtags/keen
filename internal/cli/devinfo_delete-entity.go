...
func newDevinfoDeleteEntityCmd(flags *rootFlags) *cobra.Command {
	var flagEntityType string
	var flagUpdateSequenceId string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed("entity-type") {
				allowedEntityType := []string{"commit", "branch", "pull_request"}
			...
			path = replacePathParam(path, "repositoryId", args[0])
			if len(args) < 2 {
			...
			if flagUpdateSequenceId != "" {
				params["_updateSequenceId"] = fmt.Sprintf("%v", flagUpdateSequenceId)
			}
	...
	cmd.Flags().StringVar(&flagEntityType, "entity-type", "commit", "Entity type (one of: commit, branch, pull_request)")
	cmd.Flags().StringVar(&flagUpdateSequenceId, "update-sequence-id", "", "An optional property to use to control deletion.")
