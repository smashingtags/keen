...
func newIssuetypeGetIssueTypesForProjectCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&flagProjectId, "project-id", "", "The ID of the project.")
	cmd.Flags().IntVar(&flagLevel, "level", 0, "The level of the issue type to filter by. Use: * `-1` for Subtask. * `0` for Base. * `1` for Epic.")
