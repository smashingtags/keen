...
func newWikiRemoveMemberFromGroupByGroupIdCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&flagGroupId, "group-id", "", "Id of the group whose membership is updated.")
	...
	cmd.Flags().StringVar(&flagKey, "key", "", "This parameter is no longer available and will be removed from the documentation soon. Use `accountId` instead.")
	cmd.Flags().StringVar(&flagUsername, "username", "", "This parameter is no longer available and will be removed from the documentation soon. Use `accountId` instead.")
