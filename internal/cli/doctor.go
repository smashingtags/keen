		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export JIRA_CLOUD_PLATFORM_USERNAME=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("JIRA_CLOUD_PLATFORM_USERNAME") != "" {
				authEnvSet = append(authEnvSet, "JIRA_CLOUD_PLATFORM_USERNAME")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JIRA_CLOUD_PLATFORM_USERNAME")
			}
			if os.Getenv("JIRA_CLOUD_PLATFORM_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "JIRA_CLOUD_PLATFORM_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JIRA_CLOUD_PLATFORM_PASSWORD")
			}
			switch {
