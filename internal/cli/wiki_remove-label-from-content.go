...
func newWikiRemoveLabelFromContentCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 2 {
				return usageErr(fmt.Errorf("label is required\nUsage: %s <%s>", cmd.CommandPath(), "label"))
			}
			path = replacePathParam(path, "label", args[1])
