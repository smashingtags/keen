...
func newServicedeskapiGetSlaInformationByIdCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 2 {
				return usageErr(fmt.Errorf("slaMetricId is required\nUsage: %s <%s>", cmd.CommandPath(), "slaMetricId"))
			}
			path = replacePathParam(path, "slaMetricId", args[1])
