...
func newScreensTabsMoveScreenCmd(flags *rootFlags) *cobra.Command {
	var stdinBody bool
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 3 {
				return usageErr(fmt.Errorf("pos is required\nUsage: %s <%s>", cmd.CommandPath(), "pos"))
			}
			path = replacePathParam(path, "pos", args[2])
