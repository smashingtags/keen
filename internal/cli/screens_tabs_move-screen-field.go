...
func newScreensTabsMoveScreenFieldCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&bodyAfter, "after", "", "The ID of the screen tab field after which to place the moved screen tab field. Required if `position` isn't provided.")
	cmd.Flags().StringVar(&bodyPosition, "position", "", "The named position to which the screen tab field should be moved. Required if `after` isn't provided.")
