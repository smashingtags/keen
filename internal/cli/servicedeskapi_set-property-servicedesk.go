...
func newServicedeskapiSetPropertyServicedeskCmd(flags *rootFlags) *cobra.Command {
	var stdinBody bool
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 2 {
				return usageErr(fmt.Errorf("requestTypeId is required\nUsage: %s <%s>", cmd.CommandPath(), "requestTypeId"))
			}
			path = replacePathParam(path, "requestTypeId", args[1])
			if len(args) < 3 {
